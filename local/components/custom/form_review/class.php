...
class Form1 extends CBitrixComponent
{
...
//        return;
    ...
    private function save($iblock_id, $props, $available_props)
    {
        ...
        $fields = [
            "NAME" => !empty($props['NAME']) ? $props['NAME'] : "Форма",
            "PREVIEW_TEXT" => !empty($props['PREVIEW_TEXT']) ? $props['PREVIEW_TEXT'] : "Текст",
            "IBLOCK_ID" => $iblock_id,
            "PROPERTY_VALUES" => [],
        ];
        ...
        foreach ($props as $key => $prop) {
            ...
            } else if ($key == 'PHOTO1'||$key == 'PHOTO2'||$key == 'PHOTO3') {
                $fields["PROPERTY_VALUES"]["PHOTO"]["n".$img_key] = $prop;
                $img_key++;
            }
        }
        $id = $el->Add($fields);

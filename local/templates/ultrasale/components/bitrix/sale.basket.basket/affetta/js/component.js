	...
	BX.Sale.BasketComponent = {
		...
		fillTotalBlocks: function()
		{
			...
			if (totalNodes && totalNodes.length)
			{
				...
				if (totalTemplate)
				{
					/** INKODER Убираем преформатированные рубли */
					this.result.TOTAL_RENDER_DATA.DISCOUNT_PRICE_FORMATED = cutRub(this.result.TOTAL_RENDER_DATA.DISCOUNT_PRICE_FORMATED);
					this.result.TOTAL_RENDER_DATA.PRICE_FORMATED = cutRub(this.result.TOTAL_RENDER_DATA.PRICE_FORMATED);
		...
		applyBasketResult: function(result)
		{
			...
			if (result.BASKET_ITEMS_COUNT === 0) {
		...
		createBasketItem: function(itemId)
		{
			...
			{
				/** INKODER Убираем преформатированные рубли */
				this.items[itemId].PRICE_FORMATED = cutRub(this.items[itemId].PRICE_FORMATED);
				this.items[itemId].FULL_PRICE_FORMATED = cutRub(this.items[itemId].FULL_PRICE_FORMATED);
				this.items[itemId].SUM_PRICE_FORMATED = cutRub(this.items[itemId].SUM_PRICE_FORMATED);
				this.items[itemId].SUM_FULL_PRICE_FORMATED = cutRub(this.items[itemId].SUM_FULL_PRICE_FORMATED);
		...
		getFormatPrice: function(price, currency)
		{
			...
			return BX.Currency.currencyFormat(price, currency, false);
		},

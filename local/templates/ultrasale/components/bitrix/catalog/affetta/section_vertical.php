...
if ($isFilter || $isSidebar): ?>
	<div class="catalog__sidebar catalog__sidebar_hide col-xl-3 col-lg-3 col-md-12 col-sm-12 <?=(isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' hidden-xs' : '')?>">
        <div class="filter__top-tt">
            <div>Фильтр<span class="filter_count"></span></div>
            <div><div class="filter__top-close js-filter-close"><img src="<?=SITE_TEMPLATE_PATH?>/assets/images/svg/close.svg" alt=""></div></div>
        </div>
        <?
        //sections
        ...
        if ($sectionListParams["COUNT_ELEMENTS"] === "Y")
        {
            ...
            if (false && $arParams["HIDE_NOT_AVAILABLE"] == "Y")
            {
...
<?endif?>
<div class="<?=(($isFilter || $isSidebar) ? "col-xl-9 col-lg-9 col-md-12" : "col-xs-12")?>">
	<div class="row">
		<div class="col-xs-12">
			<?
		...
		<div class="catalog_products col-xs-12">
			<?
            ...
            <?//dump($GLOBALS['tags']);
                ...
                $curPage = mb_eregi_replace("filter/.+/apply/", '', $curPage);
            ?>
            <h1><?$APPLICATION->ShowTitle(false);?></h1>

            <?
            $select = ['UF_TOP_DESCRIPTION'];
            $filter = ["IBLOCK_ID" => 13, 'ID' => $arResult["VARIABLES"]["SECTION_ID"]];
			...
			if($ar_res = $res->GetNext())
			  echo '<p>'.$ar_res['UF_TOP_DESCRIPTION'].'</p>';
            ?>

            <div class="catalog-page__content-filter js-filter-mob"><img src="<?=SITE_TEMPLATE_PATH?>/assets/images/svg/filter.svg" alt=""><span>Фильтры<span class="filter_count"></span></span></div>

            <div class="tags">
                <?foreach ($GLOBALS['tags']['VALUES'] as $tag):?>
                    <div class="tag__item <?=($tag['CHECKED'])?'active':''?>">
                        <a href="<?=$curPage?>filter/<?=strtolower($GLOBALS['tags']['CODE'])?>-is-<?=$tag['URL_ID']?>/apply/"><?=$tag['VALUE']?></a>
                    </div>
                <?endforeach?>
            </div>
...
<!--            sort line component artem-->
            ...
            $APPLICATION->IncludeComponent(
                ...
                array(
                    ...
                    'EXT_VIEW_TYPE' => (!empty($_COOKIE['catalog_view_type'])) ? $_COOKIE['catalog_view_type'] : 'tile',
...
//                   /sort_line controll
		...
		if (false && ModuleManager::isModuleInstalled("sale"))
		{
...
<?php
$APPLICATION->SetTitle($GLOBALS['ext_secrion_name'].' '.$GLOBALS['brand']['VALUE']);
